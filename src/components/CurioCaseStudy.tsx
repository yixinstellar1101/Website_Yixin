...
const overviewSlides = curioSlides.slice(0, 4);
const systemSlides = curioSlides.slice(4, 10);
const phoneSlides = curioSlides.slice(10, 18);
const validationSlides = curioSlides.slice(18);
...
function NarrativeGrid({ slides, onOpen }: { slides: CurioSlide[]; onOpen: (index: number) => void }) {
  return (
...
function PhoneShowcase({ slides, onOpen }: { slides: CurioSlide[]; onOpen: (index: number) => void }) {
  return (
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {slides.map((slide, index) => (
        <div key={slide.id} className={index === 0 || index === slides.length - 1 ? "xl:col-span-3" : ""}>
          <div className="mx-auto max-w-[360px] xl:max-w-none">
            <SlideCard slide={slide} slideIndex={Number(slide.id) - 1} onOpen={onOpen} />
          </div>
        </div>
      ))}
    </div>
...
export function CurioCaseStudy({ project, videoSrc, pdfSrc }: CurioCaseStudyProps) {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden">
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-[38rem] bg-[radial-gradient(circle_at_14%_82%,rgba(255,202,173,0.38),transparent_28%),radial-gradient(circle_at_84%_88%,rgba(180,204,255,0.4),transparent_30%),radial-gradient(circle_at_72%_58%,rgba(223,201,255,0.3),transparent_26%)]" />
            ...
            <div>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-3">
                <div className="rounded-[20px] border border-white/70 bg-white/64 p-4">
                  ...
                  <p className="mt-2 text-sm font-medium text-ink">AI MVP</p>
                </div>
                <div className="rounded-[20px] border border-white/70 bg-white/64 p-4">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[rgba(11,34,66,0.46)]">Validation</p>
                  <p className="mt-2 text-sm font-medium text-ink">100+ Activations</p>
                </div>
                <div className="rounded-[20px] border border-white/70 bg-white/64 p-4">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[rgba(11,34,66,0.46)]">Focus</p>
                  <p className="mt-2 text-sm font-medium text-ink">Multi-Agent Live Loop</p>
                </div>
              </div>
            </div>
        ...
        <div className="mt-10 space-y-10">
          ...
          >
            <div className="grid gap-8 lg:grid-cols-[0.36fr_0.64fr] lg:items-start">
              <div className="lg:sticky lg:top-32">
                <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[rgba(11,34,66,0.46)]">Overview</p>
                <h2
                ...
                >
                  Turning Cultural Artifacts Into A Live AI Discussion Format
                </h2>
                <p className="mt-4 text-base leading-8 text-[rgba(11,34,66,0.72)]">
                  Curio started from a simple question: could a cultural object become the center of a live, multi-agent conversation instead of a static explanation? The case study page now uses the exported slides as visual evidence, while the HTML narrative explains the product framing, experience design, and MVP validation story around them.
                </p>
                <p className="mt-4 text-base leading-8 text-[rgba(11,34,66,0.72)]">
                  Instead of flattening the story into one long document, the page turns the deck into a readable sequence with breathing room, sectioning, and direct visual focus on the strongest screens.
                </p>
              </div>
              <NarrativeGrid slides={overviewSlides} onOpen={setActiveSlideIndex} />
            </div>
          </motion.section>
          ...
          >
            <div className="grid gap-8 lg:grid-cols-[0.38fr_0.62fr] lg:items-start">
              <div className="lg:sticky lg:top-32">
                <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[rgba(11,34,66,0.46)]">Product Loop</p>
                <h2
                  className="mt-4 text-[clamp(1.9rem,3vw,3rem)] leading-[1.02] text-ink"
                  style={{ fontFamily: "ABC Ginto Normal Medium, Inter, sans-serif", fontWeight: 500 }}
                >
                  Upload, Frame, Cast, And Orchestrate The Conversation
                </h2>
                <p className="mt-4 text-base leading-8 text-[rgba(11,34,66,0.72)]">
                  The interaction flow was shaped to feel legible from the first action. A user uploads an artifact or object, the system identifies a cultural angle, generates agent roles, and stages a live discussion that is easier to understand than a hidden prompt chain.
                </p>
                <div className="mt-6 space-y-3">
                  <div className="rounded-[18px] border border-white/70 bg-white/70 px-4 py-4 text-sm leading-7 text-[rgba(11,34,66,0.72)]">
                    Strong first input: any object can become the entry point.
                  </div>
                  <div className="rounded-[18px] border border-white/70 bg-white/70 px-4 py-4 text-sm leading-7 text-[rgba(11,34,66,0.72)]">
                    Agent roles create interpretive depth instead of one generic answer.
                  </div>
                  <div className="rounded-[18px] border border-white/70 bg-white/70 px-4 py-4 text-sm leading-7 text-[rgba(11,34,66,0.72)]">
                    Live-room framing makes the system social, participatory, and memorable.
                  </div>
                </div>
              </div>
              <NarrativeGrid slides={systemSlides} onOpen={setActiveSlideIndex} />
            </div>
          </motion.section>
          ...
          >
            <div className="grid gap-8">
              <div className="max-w-[760px]">
                <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[rgba(11,34,66,0.46)]">Phone Experience</p>
                <h2
                  className="mt-4 text-[clamp(1.9rem,3vw,3rem)] leading-[1.02] text-ink"
                  style={{ fontFamily: "ABC Ginto Normal Medium, Inter, sans-serif", fontWeight: 500 }}
                >
                  Presenting The UI Screens As Polished Product Moments
                </h2>
                <p className="mt-4 text-base leading-8 text-[rgba(11,34,66,0.72)]">
                  The exported mobile slides are shown here as key interface moments, giving the phone UI enough room to stay readable on desktop, tablet, and mobile while preserving the visual storytelling from the original review deck.
                </p>
              </div>
              <PhoneShowcase slides={phoneSlides} onOpen={setActiveSlideIndex} />
            </div>
          </motion.section>
          ...
          >
            <div className="grid gap-8 lg:grid-cols-[0.36fr_0.64fr] lg:items-start">
              <div className="lg:sticky lg:top-32">
                <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[rgba(11,34,66,0.46)]">Validation</p>
                <h2
                  className="mt-4 text-[clamp(1.9rem,3vw,3rem)] leading-[1.02] text-ink"
                  style={{ fontFamily: "ABC Ginto Normal Medium, Inter, sans-serif", fontWeight: 500 }}
                >
                  Shipping An MVP To Test Demand, Resonance, And Next Steps
                </h2>
                <p className="mt-4 text-base leading-8 text-[rgba(11,34,66,0.72)]">
                  This concept was not framed as a speculative deck only. The goal was to get a working interactive MVP into users' hands, observe activation behavior, and learn whether the experience felt culturally meaningful instead of merely technically interesting.
                </p>
                <p className="mt-4 text-base leading-8 text-[rgba(11,34,66,0.72)]">
                  Those signals helped validate the demand for a more conversational and agentic format, while clarifying where a future product version would need stronger authoring, moderation, and retention loops.
                </p>
              </div>
              <NarrativeGrid slides={validationSlides} onOpen={setActiveSlideIndex} />
            </div>
          </motion.section>
        </div>
