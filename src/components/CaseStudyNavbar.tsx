...
export function CaseStudyNavbar({
  ...
  const letsTalkLabel = pageCopy.letsTalk[locale];
                    ...
                    <nav className="mx-5 hidden flex-1 items-center justify-center gap-5 lg:flex xl:gap-7">
                      {navItems.map((item) => (
                        <Link
                          key={item.href}
                          href={item.href}
                          className={navButtonClass(item.href === activeHref)}
                        >
                          {item.label[locale]}
                        </Link>
                      ))}
                    ...
                    <div className="grid gap-2">
                      {navItems.map((item) => (
                        <Link
                          ...
                          className={navButtonClass(item.href === activeHref) + " rounded-full px-4 py-3 text-left"}
                        >
                          {item.label[locale]}
                        </Link>
                      ))}
